  ...
  try {
    ...
    if ('name' in body) {
      ...
      await db.transaction(async transaction => {
        await transaction.update(events).set({ name: data.name, date: data.date, endDate: data.endDate, location: data.location, organizer: data.organizer, type: data.type, nextActions: data.nextActions, generalNotes: data.generalNotes }).where(eq(events.id, id));
