...
export const eventFormSchema = z.object({ name: required('Nama event wajib diisi'), date: required('Tanggal wajib diisi'), endDate: text, location: required('Lokasi wajib diisi'), organizer: text, type: text, networking: z.array(personSchema).catch([]), prospects: z.array(prospectSchema).catch([]), nextActions: z.array(z.string()).catch([]), generalNotes: text }).superRefine((data, context) => {
  if (data.endDate && data.date > data.endDate) context.addIssue({ code: z.ZodIssueCode.custom, message: 'Tanggal akhir harus setelah atau sama dengan tanggal awal', path: ['endDate'] });
...
export function cleanEventForm(data: EventFormValues): EventFormValues { return { ...data, name: data.name.trim(), date: data.date.trim(), endDate: data.endDate.trim(), location: data.location.trim(), organizer: data.organizer.trim(), type: data.type.trim(), networking: data.networking.filter(hasPersonData), prospects: data.prospects.filter(hasProspectData), generalNotes: data.generalNotes.trim() }; }
